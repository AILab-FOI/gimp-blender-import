...
re_imgsize = re.compile( r'IMGSIZE ([0-9]+)x([0-9]+)' )

bl_info = {
    "name": "Import GIMP Image as Layers (.xcf, .xfcgz, .xfcbz2 etc.)",
    "author": "Markus Schatten (AILab-FOI)",
    "version": (1, 0, 0),
    "blender": (2, 80, 0),
    "location": "File > Import > Import GIMP image layers as planes",
    "description": "Imports GIMP multilayer image files as a series of multiple planes. Based on Import Images as Planes by Florian Meyer (tstscr), mont29, matali and Ted Schundler (SpkyElctrc) as well as on Import GIMP Image to Scene (.xcf, .xjt) by Daniel Salazar (ZanQdo).",
    "warning": "Requires GIMP installed and on system path. Currently offsets and opacity of original layers are not preserved.",
    "tracker_url": "https://github.com/AILab-FOI/gimp-blender-import/issues",
    "category": "Import-Export"}


'''
;; Script-fu script for getting layer data.
;; Stolen partly from: https://github.com/pft/script-fu/
;; SAVE-LAYERS "filename.xcf"
;; OUTPUT:
;; "layer name":TYPE:OPACITY:OFFSET:WIDTH:HEIGHT:VISIBLE
;;
;; TYPE in { RGB-IMAGE (0), RGBA-IMAGE (1), GRAY-IMAGE (2), GRAYA-IMAGE (3), INDEXED-IMAGE (4), INDEXEDA-IMAGE (5) }
;;
;; OPACITY in 0.0 - 100.0
;;
;; OFFSET (X Y) X,Y are integers
;;
...
(define (save-layers img)
  (let* ((image (car (gimp-file-load RUN-NONINTERACTIVE img img)))
    ...
    (write "!----------BEGIN----------!" (open-output-file "/dev/stderr"))
    (write-char #\Newline (open-output-file "/dev/stderr"))
    (let loop ((layers (vector->list (cadr (gimp-image-get-layers image)))))
      (unless (null? layers)
		...
		(let ((mode (gimp-drawable-type (car layers))))
		  (write mode (open-output-file "/dev/stderr")))
		(write-char #\: (open-output-file "/dev/stderr"))
		(let ((opacity (gimp-layer-get-opacity (car layers))))
		  (write opacity (open-output-file "/dev/stderr")))
		(write-char #\: (open-output-file "/dev/stderr"))
		(let ((offset (gimp-drawable-offsets (car layers))))
		  (write offset (open-output-file "/dev/stderr")))
		(write-char #\: (open-output-file "/dev/stderr"))
		(let ((width (gimp-drawable-width (car layers))))
		  (write width (open-output-file "/dev/stderr")))
		(write-char #\: (open-output-file "/dev/stderr"))
		(let ((height (gimp-drawable-height (car layers))))
		  (write height (open-output-file "/dev/stderr")))
		(write-char #\: (open-output-file "/dev/stderr"))
		(let ((visible (gimp-item-get-visible (car layers))))
		  (write visible (open-output-file "/dev/stderr")))
		(write-char #\: (open-output-file "/dev/stderr"))
		(write (car (gimp-drawable-get-name (car layers))) (open-output-file "/dev/stderr"))
		(write-char #\Newline (open-output-file "/dev/stderr"))
		(gimp-image-delete img))
	      (loop (cdr layers)))))
  (write "!----------END----------!" (open-output-file "/dev/stderr"))
  (write-char #\Newline (open-output-file "/dev/stderr")))
"""
...
def get_type( num ):
    if num == 0:
        ...
        return 'GRAY-IMAGE', False
    elif num == 3:
        return 'GRAY-IMAGE', True
    elif num == 4:
        return 'INDEXED-IMAGE', False
    elif num == 5:
        return 'INDEXED-IMAGE', True
    raise Exception( 'Type index out of range 0 - 5, got %d' % num )

def get_img_size( fname ):
    cmd = """gimp -idf -b '(let* ((image (car (gimp-file-load RUN-NONINTERACTIVE "%s" "%s"))) (img-width (car (gimp-image-width image))) (img-height (car (gimp-image-height image)))) (gimp-message (string-append "IMGSIZE " (number->string img-height) "x" (number->string img-width))))' -b '(gimp-quit 0)'""" % ( fname, fname )
    args = shlex.split( cmd )
    
    gb = sp.run( args, capture_output = True )

    output = gb.stderr.decode( 'utf-8' )

    x, y = [ int( i ) for i in re_imgsize.findall( output )[ 0 ] ]

    return x, y
    

def export_images( fname ):
    width, height = get_img_size( fname )
    
    path = Path( fname ).stem
    ...
    cmd = """gimp -idf -b '%s (save-layers "%s")' -b '(gimp-quit 0)'""" % ( script_fu % savepath, fname )
    ...
    for line in output.split( '\n' ):
        ...
        if start and not end:
            ...
            parts = line.split( ':', 7 )
            layer = {}
            typ, alpha = get_type( int( parts[ 0 ][ 1:-1 ] ) )
            layer[ 'type' ] = typ
            layer[ 'alpha' ] = alpha
            layer[ 'opacity' ] = float( parts[ 1 ][ 1:-1 ] ) * 0.01
            offx, offy = [ int( i ) for i in parts[ 2 ][ 1:-1 ].split()  ]
            layer[ 'offsetX' ] = offx
            layer[ 'offsetY' ] = offy
            layer[ 'width' ] = int( parts[ 3 ][ 1:-1 ] )
            layer[ 'height' ] = int( parts[ 4 ][ 1:-1 ] )
            layer[ 'visible' ] = bool( int( parts[ 5 ][ 1:-1 ] ) )
            layer[ 'name' ] = parts[ 6 ][ 1:-1 ]
            layer[ 'filename' ] = os.path.join( savepath, layer[ 'name' ] + '.png' )
...
def main(File, Path, LayerViewers, MixerViewers, LayerOffset,
    ...
    for layer in layers:
        if layer[ 'visible' ]:
            ...
            l[ 'LayerMode' ] = layer[ 'type' ]
            l[ 'LayerOpacity' ] = layer[ 'opacity' ]
            l[ 'LayerName' ] = layer[ 'name' ]
            l[ 'LayerNameShort' ] = layer[ 'name' ].split( '[' )[ 0 ]
            l[ 'RenderLayer' ] = layer[ 'name' ]
            l[ 'LayerCoords' ] = [ layer[ 'width' ], layer[ 'height' ], layer[ 'offsetX' ], layer[ 'offsetY' ] ]
            l[ 'HasAlpha' ] = layer[ 'alpha' ]

            IMGs.append( l )
    
    ResX, ResY = get_img_size( fullpath )
...
if BLENDER:
    ...
    class IMPORT_GIMP_OT_to_plane( Operator, AddObjectHelper ):
        bl_idname = "import_gimp.to_plane"
        bl_label = "Import GIMP image layers as planes"
        ...
        filter_glob: StringProperty(
            default='*.xcf;*.xcfgz;*.xcf.gz;*.xcfbz2;*.xcf.bz2',
            options={'HIDDEN'}
        )
        ...
        import_hidden: BoolProperty(
            name="Import Hidden Layers", default=False,
            description="Import Layers Which Have Been Hidden In GIMP Image"
        )
        ...
        keep_offsets: BoolProperty( name="Keep Offsets", default=True, description="Keep Offsets Of Layers From The Original GIMP Image" )
        ...
        def execute(self, context):
            ...
            for fl in self.files:
                imgs.extend( export_images( os.path.join( self.directory, fl.name ) ) )

            self.files.clear()

            for i in imgs:
                if i[ 'visible' ] or self.import_hidden:
                    elem = self.files.add()
                    elem.name = i[ 'filename' ]
        ...
        def fix_offset( self, context, img, plane ):
            # Fix the offset of a GIMP layer
            # Compute size of original GIMP image
            fake_spec = ImageSpec( None, ( img[ 'image width' ], img[ 'image height' ], 0, 0, 0 ) )
            ox, oy = self.compute_plane_size( context, fake_spec )

            # Compute size of the layer
            lay_spec = ImageSpec( None, ( img[ 'width' ], img[ 'height' ] ), 0, 0, 0 ) )
            lx, ly, = self.compute_plane_size( context, lay_spec )

            # Get axis of the plane
            axis = self.axis_id_to_vector[ self.align_axis ]
            # Get adequate orthogonal axis
            # ???
            oaxis = [ 0 if offset_axis[ i ] else 1 for i in ( 0, 1 ) ] # ???

            # Compute move x vector
            move_x = axis * ( ( img[ 'offsetX' ] / img[ 'image width' ] ) * ox )

            # Compute move y vector
            move_y = oaxis * ( ( ( img[ 'image height' ] - img[ 'offsetY' ] - img[ 'height' ] ) / img[ 'image height' ] ) * oy )

            # Move plane
            # ???
            self.move_along_vector( plane, move_x )
            self.move_along_vector( plane, move_y )

        def move_along_vector( self, obj, vector ):
            #???
            vector.normalize()
            ...
            for pos in range( 0, 10 ):
                for i in range( 0, 3 ):
                    obj.location[ i ] = objloc[ i ] + ( pos * vector[ i ] )
    ...
    class NO_IMPORT_GIMP_OT_to_plane( Operator, AddObjectHelper ):
        bl_idname = "import_gimp.to_plane"
        bl_label = "Import GIMP image layers as planes"
        bl_options = {'REGISTER', 'PRESET', 'UNDO'}
        ...
        def execute( self, context ):
            ...
            for img in imgs:
                try:
                    bpy.ops.import_image.to_plane( shader='SHADELESS', files=[ { 'name':os.path.join( self.directory, img[ 'filename' ] ) } ] )
                except Exception as e:
                    print( e )
            return {'FINISHED'}

    def import_gimp_button( self, context ):
        self.layout.operator( IMPORT_GIMP_OT_to_plane.bl_idname, text="Import GIMP as planes", icon='TEXTURE')


    classes = (
        IMPORT_GIMP_OT_to_plane,
    )
    ...
    def register():
        ...
        bpy.types.TOPBAR_MT_file_import.append( import_gimp_button )
        bpy.types.VIEW3D_MT_image_add.append( import_gimp_button )
    ...
    def unregister():
        bpy.types.TOPBAR_MT_file_import.remove( import_gimp_button )
        bpy.types.VIEW3D_MT_image_add.remove( import_gimp_button )
...
if __name__ == "__main__":
    print( 'Testing' )
    export_images( 'test.xcf' )
    get_img_size( 'test.xcf' )
